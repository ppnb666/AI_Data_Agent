...
> 基于大语言模型（LLM）的企业数据智能分析 Agent，支持任意行业、任意格式（Excel / CSV / JSON）数据文件
...
| **自动清洗** | 根据质量报告自动清洗脏数据（前后缀格式由样本统计驱动） |
| **通用格式适配** | 任意 .xlsx/.xls/.csv/.json，字段名不同、Sheet不同、行业不同均自动适配 |
...
> 支持 `DISABLE_LLM_SCHEMA=1` 环境变量：关闭 Schema 识别的 LLM 调用，降级为纯关键词识别（省 token / 离线场景）。
...
或上传任意格式数据文件（`.xlsx/.xls/.csv/.json`），对任意行业提问，例如：
...
通过 `/preview` 接口上传任意数据文件（Excel/CSV/JSON），系统自动猜测字段含义（LLM 角色识别 → 关键词兜底），用户确认后保存映射，后续查询自动适配该文件结构。
